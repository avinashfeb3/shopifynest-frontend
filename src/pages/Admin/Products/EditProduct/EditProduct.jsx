...
const EditProduct = () => {
  ...
  // get Product API Section Start
  const getProduct = async () => {
    try {
      const { data, success } = await axios.get(`/admin/products/${id}`);

      if (success && data?.product) {
        const product = data.product;
        setProduct(product);

        // Helper to get ID from possible fields
        const getID = (obj) => obj?._id || obj?.id || (typeof obj === "string" ? obj : "");
        const categoryId = getID(product.category);
        const subcategoryId = getID(product.subcategory) || getID(product.subCategory) || getID(product.sub_category);
        ...
        reset({
          ...
          is_featured: product.is_featured || product.isFeatured?.toLowerCase?.() || "",
        });
      }
  ...
  const getSubCategories = async (categoryId) => {
    try {
      const response = await axios.get(
        `/admin/sub-categories?category_id=${categoryId}`,
      );
      if (response.success && response.data) {
        setSubCategories(
          Array.isArray(response.data)
                    ...
                    <div className="flex flex-col">
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="grid grid-cols-5 gap-2 mt-3">
                          {product && product.gallery.map((image, index) => (
                            <div
                              key={index}
